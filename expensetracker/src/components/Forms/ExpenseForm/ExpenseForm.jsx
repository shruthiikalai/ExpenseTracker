...
export default function ExpenseForm({ setIsOpen, expenseList, setExpenseList, editId, setBalance, balance }) {
    ...
    const handleAdd = (e) => {
        ...
        enqueueSnackbar('Expense added successfully', { variant: 'success' }); // Success notification
        setIsOpen(false);
    };
    ...
    const handleEdit = (e) => {
        ...
        const updated = expenseList.map((item) => {
            if (item.id === editId) {
                ...
                enqueueSnackbar('Expense updated successfully', { variant: 'success' }); // Success notification
